...
/**
 * A UDP Sink {@link DataSource}.
 */
public class UdpDataSinkSource extends BaseDataSource implements UdpDataSink {
    /**
    ...
    /**
     * Thrown when an error is encountered when trying to read from a {@link UdpDataSinkSource}.
     */
    public static final class UdpDataSinkSourceException extends IOException {

        public UdpDataSinkSourceException(IOException cause) {
            super(cause);
        }
    ...
    public UdpDataSinkSource() {
        this(DEFAULT_MAX_PACKET_SIZE, DEFAULT_SOCKET_TIMEOUT_MILLIS);
    }
     ...
     *
     * @param maxPacketSize       The maximum datagram packet size, in bytes.
     * @param socketTimeoutMillis The socket timeout in milliseconds. A timeout of zero is
    ...
    @Override
    public long open(DataSpec dataSpec) throws UdpDataSinkSourceException {
        uri = dataSpec.uri;
        ...
        try {
            ...
            throw new UdpDataSinkSourceException(e);
        }
    ...
    @Override
    public int read(byte[] buffer, int offset, int readLength) throws UdpDataSinkSourceException {
        if (readLength == 0) {
            return 0;
        }
