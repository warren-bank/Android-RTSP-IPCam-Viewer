...
public final class RtspSampleStreamWrapper implements Loader.Callback<RtspSampleStreamWrapper.MediaStreamLoadable>, SequenceableLoader, ExtractorOutput, SampleQueue.UpstreamFormatChangedListener, MediaSession.EventListener, RtcpOutputReportDispatcher.EventListener {
    ...
    private void sendPunchPacket(String host, int port) {
        try {
            ...
            ((RtpDataSource) loadable.dataSource).writeTo(MAGIC_BYTES, 0, MAGIC_BYTES.length, InetAddress.getByName(host), port);
     ...
     * @param enabledState     True if the sample queue is being enabled, or false if it's being
     *                         disabled.
     */
    ...
    /* package */ abstract class MediaStreamLoadable implements Loader.Loadable {
        /**
         * The maximum length of an datagram data packet size, in bytes. 65535 bytes minus IP header
         * (20 bytes) and UDP header (8 bytes)
         */
    ...
    /* package */ final class UdpMediaStreamLoadable extends MediaStreamLoadable {
        ...
        protected DataSource buildAndOpenDataSource() throws IOException {
            ...
            if (Transport.RTP_PROTOCOL.equals(transport.transportProtocol())) {
                ...
                dataSource = new RtpDataSource(MAX_UDP_PACKET_SIZE);
                isUdpSchema = true;
            }
            ...
            DataSpec dataSpec = new DataSpec(Uri.parse((isUdpSchema ? "udp" : "rtp") + "://" +
                    IPV4_ANY_ADDR + ":" + localPort), DataSpec.FLAG_ALLOW_CACHE_FRAGMENTATION);
