 ...
 * at the start of playback. The library modules provide default implementations for progressive
 * media files ({@link ProgressiveMediaSource}), DASH (DashMediaSource), SmoothStreaming
 * (SsMediaSource) and HLS (HlsMediaSource), an implementation for loading single media samples
 * ({@link SingleSampleMediaSource}) that's most often used for side-loaded subtitle files, and
 * implementations for building more complex MediaSources from simpler ones ({@link
 * MergingMediaSource}, {@link ConcatenatingMediaSource}, {@link LoopingMediaSource} and {@link
 * ClippingMediaSource}).
 * <li><b>{@link Renderer}</b>s that render individual components of the media. The library
 * provides default implementations for common media types ({@link MediaCodecVideoRenderer}, {@link
 * MediaCodecAudioRenderer}, {@link TextRenderer} and {@link MetadataRenderer}). A Renderer consumes
 * media from the MediaSource being played. Renderers are injected when the player is created.
 ...
 * ({@link DefaultTrackSelector}) suitable for most use cases. A TrackSelector is injected when the
 * player is created.
 ...
 * DefaultLoadControl}) suitable for most use cases. A LoadControl is injected when the player is
 * created.
 * </ul>
 *
 ...
 * majority of cases this should be the application's main thread. Using the application's main
 * thread is also a requirement when using ExoPlayer's UI components or the IMA extension. The
 * thread on which an ExoPlayer instance must be accessed can be explicitly specified by passing a
 * `Looper` when creating the player. If no `Looper` is specified, then the `Looper` of the thread
 * that the player is created on is used, or if that thread does not have a `Looper`, the `Looper`
 * of the application's main thread is used. In all cases the `Looper` of the thread from which the
 * player must be accessed can be queried using {@link #getApplicationLooper()}.
 * <li>Registered listeners are called on the thread associated with {@link
 * #getApplicationLooper()}. Note that this means registered listeners are called on the same thread
 * which must be used to access the player.
...
public interface ExoPlayer extends Player {
    ...
    final class Builder {
         ...
         *
         * <p>Note that this constructor is only useful if you try to ensure that ExoPlayer's
         * default
         * components can be removed by ProGuard or R8. For most components except renderers, there
         * is only a marginal benefit of doing that.
         *
         * @param renderers          The {@link Renderer Renderers} to be used by the player.
         * @param trackSelector      A {@link TrackSelector}.
         * @param loadControl        A {@link LoadControl}.
         * @param bandwidthMeter     A {@link BandwidthMeter}.
         * @param looper             A {@link Looper} that must be used for all calls to the
         *                           player.
    ...
    /**
     * Retries a failed or stopped playback. Does nothing if the player has been reset, or if
     * playback has not failed or been stopped.
     */
    void retry();
    ...
    /**
     * Prepares the player to play the provided {@link MediaSource}, optionally resetting the
     * playback position the default position in the first {@link Timeline.Window}.
     *
     * @param mediaSource   The {@link MediaSource} to play.
     * @param resetPosition Whether the playback position should be reset to the default position in
     *                      the first {@link Timeline.Window}. If false, playback will start from
     *                      the position defined by {@link #getCurrentWindowIndex()} and {@link
     *                      #getCurrentPosition()}.
     * @param resetState    Whether the timeline, manifest, tracks and track selections should be
     *                      reset. Should be true unless the player is being prepared to play the
     *                      same media as it was playing previously (e.g. if playback failed and is
     *                      being retried).
     */
     ...
     * PlayerMessage#getType()} is 0 and the default {@link PlayerMessage#getPayload()} is null. If
     * a position is specified with {@link PlayerMessage#setPosition(long)}, the message will be
     ...
     * the same player instance must be used to play multiple pieces of content, and there must be
     * gaps between the playbacks (i.e. {@link #stop} is called to halt one playback, and {@link
     * #prepare} is called some time later to start a new one).
     * </ul>
     *
     * <p>Note that foreground mode is <em>not</em> useful for switching between content without
     * gaps
     * between the playbacks. For this use case {@link #stop} does not need to be called, and simply
     * calling {@link #prepare} for the new media will cause limited resources to be retained even
     * if foreground mode is not enabled.
     *
     * <p>If foreground mode is enabled, it's the application's responsibility to disable it when
     * the
     * conditions described above no longer hold.
     *
     * @param foregroundMode Whether the player is allowed to keep limited resources even when in
     *                       the idle state.
     */
