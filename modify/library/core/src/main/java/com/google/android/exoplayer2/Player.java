 ...
 * type (e.g. audio, video or text). The number of renderers and their respective track types can be
 * obtained by calling {@link #getRendererCount()} and {@link #getRendererType(int)}.
 * </li>
...
public interface Player {
    ...
    interface AudioComponent {
         ...
         * cannot
         * be set directly on the underlying audio track. In this case, the usage will be mapped
         * onto an equivalent stream type using {@link Util#getStreamTypeForAudioUsage(int)}.
         *
    ...
    interface VideoComponent {
         ...
         *
         * <p>This method is intended to be called by the same component that sets the {@link
         * Surface}
         * onto which video will be rendered. If using ExoPlayer's standard UI components, this
         * method should not be called directly from application code.
         *
        ...
        /**
         * Clears the listener which receives video frame metadata events if it matches the one
        ...
        /**
         * Clears any {@link Surface}, {@link SurfaceHolder}, {@link SurfaceView} or {@link
         * TextureView} currently set on the player.
         */
        void clearVideoSurface();

        /**
         * Clears the {@link Surface} onto which video is being rendered if it matches the one
         * passed. Else does nothing.
         *
         ...
         * SurfaceHolder} then it's recommended to use {@link #setVideoSurfaceView(SurfaceView)},
         * {@link #setVideoTextureView(TextureView)} or {@link #setVideoSurfaceHolder(SurfaceHolder)}
         * rather than this method, since passing the holder allows the player to track the
        ...
        /**
         * Sets the video decoder output buffer renderer. This is intended for use only with
         * extension renderers that accept {@link C#MSG_SET_VIDEO_DECODER_OUTPUT_BUFFER_RENDERER}.
         * For most use cases, an output surface or view should be passed via {@link
         * #setVideoSurface(Surface)} or {@link #setVideoSurfaceView(SurfaceView)} instead.
         *
         * @param videoDecoderOutputBufferRenderer The video decoder output buffer renderer, or
         *                                         {@code null} to clear the output buffer
         *                                         renderer.
         */
         ...
         * nothing.
         *
         * @param videoDecoderOutputBufferRenderer The video decoder output buffer renderer to
         *                                         clear.
         */
        void clearVideoDecoderOutputBufferRenderer(
                @Nullable VideoDecoderOutputBufferRenderer videoDecoderOutputBufferRenderer);
    ...
    interface EventListener {
         ...
         * occurred. For example, the current period index may have changed as a result of periods
         * being added or removed from the timeline. This will <em>not</em> be reported via a
         * separate call to {@link #onPositionDiscontinuity(int)}.
         *
         ...
         *                        length {@link #getRendererCount()}, but may contain null
         *                        elements.
         */
         ...
         * discontinuity occurs when the current window or period index changes (as a result of
         * playback transitioning from one period in the timeline to the next), or when the playback
         * position jumps within the period currently being played (as a result of a seek being
         * performed, or when the source introduces a discontinuity internally).
         *
         * <p>When a position discontinuity occurs as a result of a change to the timeline this
         * method
         * is <em>not</em> called. {@link #onTimelineChanged(Timeline, int)} is called in this
         * case.
         *
        ...
        /**
         * Called when the current playback parameters change. The playback parameters may change
         * due to a call to {@link #setPlaybackParameters(PlaybackParameters)}, or the player itself
         ...
         * guaranteed to happen after any necessary changes to the player state were reported to
    ...
    /**
     * @deprecated Use {@link EventListener} interface directly for selective overrides as all
     * methods are implemented as no-op default methods.
     */
    @Deprecated
     ...
     * One of {@link #PLAYBACK_SUPPRESSION_REASON_NONE} or {@link #PLAYBACK_SUPPRESSION_REASON_TRANSIENT_AUDIO_FOCUS_LOSS}.
     */
    @Documented
    @Retention(RetentionPolicy.SOURCE)
    @IntDef({
    ...
    /**
     * Timeline or manifest changed as a result of an dynamic update introduced by the played
     * media.
     */
    ...
    /**
     * Returns the component of this player for metadata output, or null if metadata is not
     * supported.
     */
    @Nullable
    ...
    /**
     * Register a listener to receive events from the player. The listener's methods will be called
     * on the thread that was used to construct the player. However, if the thread used to construct
     * the player does not have a {@link Looper}, then the listener will be called on the main
     * thread.
     *
     * @param listener The listener to register.
     */
    ...
    /**
     * Returns the reason why playback is suppressed even though {@link #getPlayWhenReady()} is
     * {@code true}, or {@link #PLAYBACK_SUPPRESSION_REASON_NONE} if playback is not suppressed.
     *
     ...
     * is re-prepared.
     *
     * <p>Note that this method will always return {@code null} if {@link #getPlaybackState()} is
     * not
     * {@link #STATE_IDLE}.
     *
    ...
    /**
     * Sets whether playback should proceed when {@link #getPlaybackState()} == {@link
     * #STATE_READY}.
     * <p>
    ...
    /**
     * Seeks to the default position associated with the specified window. The position can depend
     * on the type of media being played. For live streams it will typically be the live edge of the
     * window. For other streams it will typically be the start of the window.
     *
     ...
     *                                      {@code windowIndex} is not within the bounds of the
     *                                      current timeline.
     */
     ...
     * current repeat mode and whether shuffle mode is enabled. Does nothing if {@link #hasNext()}
     * is {@code false}.
     */
    void next();
     ...
     * default, {@link PlaybackParameters#DEFAULT}, which means there is no speed or pitch
     * adjustment.
     *
     ...
     *
     * <p>Calling this method will cause the playback state to transition to {@link #STATE_IDLE}.
     * The
    ...
    /**
     * Returns the current manifest. The type depends on the type of media being played. May be
     * null.
     */
    @Nullable
    ...
    /**
     * Returns the index of the previous timeline window to be played, which may depend on the
     * current repeat mode and whether shuffle mode is enabled. Returns {@link C#INDEX_UNSET} if the
     * window currently being played is the first window.
     */
    ...
    /**
     * Returns whether the current window is live, or {@code false} if the {@link Timeline} is
     * empty.
     *
     ...
     * window in milliseconds, or {@link C#TIME_UNSET} if the duration is not known. If there is no
     * ad playing, the returned duration is the same as that returned by {@link #getDuration()}.
     */
     ...
     * playing, the returned position is the same as that returned by {@link
     * #getCurrentPosition()}.
     */
    long getContentPosition();

    /**
     * If {@link #isPlayingAd()} returns {@code true}, returns an estimate of the content position
     * in the current content window up to which data is buffered, in milliseconds. If there is no
     * ad playing, the returned position is the same as that returned by {@link
     * #getBufferedPosition()}.
     */
