...
public interface Renderer extends PlayerMessage.Target {
     ...
     * rendering (e.g. media decoders), for use if it's subsequently enabled. {@link #reset()} can
     * be called to force the renderer to release these resources.
     */
    ...
    int STATE_ENABLED = 1;
    /**
     * The renderer is started. Calls to {@link #render(long, long)} will cause media to be
     * rendered.
     */
    int STATE_STARTED = 2;
     ...
     * return {@link C#TRACK_TYPE_VIDEO}, an audio renderer will return {@link C#TRACK_TYPE_AUDIO},
     * a text renderer will return {@link C#TRACK_TYPE_TEXT}, and so on.
     *
    ...
    /**
     * Returns the playback position up to which the renderer has read samples from the current
     * {@link SampleStream}, in microseconds, or {@link C#TIME_END_OF_SOURCE} if the renderer has
     * read the current {@link SampleStream} to the end.
     *
    ...
    /**
     * Throws an error that's preventing the renderer from reading from its {@link SampleStream}.
     ...
     * work toward being ready to render the {@link SampleStream} when the renderer is started. It
     * may also render the very start of the media, for example the first frame of a video stream.
     * If the renderer is in the {@link #STATE_STARTED} state then calls to this method will render
     * the {@link SampleStream} in sync with the specified media positions.
     * <p>
     ...
     *                          measured at the start of the current iteration of the rendering
     *                          loop.
     * @throws ExoPlaybackException If an error occurs.
     */
