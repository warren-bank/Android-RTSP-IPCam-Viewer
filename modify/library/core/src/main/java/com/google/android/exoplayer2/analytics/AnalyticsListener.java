...
public interface AnalyticsListener {
    ...
    final class EventTime {
        ...
        /**
         * Window index in the {@link #timeline} this event belongs to, or the prospective window
         * index if the timeline is not yet known and empty.
         */
        public final int windowIndex;

        /**
         * Media period identifier for the media period this event belongs to, or {@code null} if
         * the event is not associated with a specific media period.
         */
        @Nullable
        ...
        /**
         * Total buffered duration from {@link #currentPlaybackPositionMs} at the time of the event,
         * in milliseconds. This includes pre-buffered data for subsequent ads and windows.
         */
        public final long totalBufferedDurationMs;

        /**
         * @param realtimeMs                Elapsed real-time as returned by {@code
         *                                  SystemClock.elapsedRealtime()} at the time of the event,
         *                                  in milliseconds.
         * @param timeline                  Timeline at the time of the event.
         * @param windowIndex               Window index in the {@link #timeline} this event belongs
         *                                  to, or the prospective window index if the timeline is
         *                                  not yet known and empty.
         * @param mediaPeriodId             Media period identifier for the media period this event
         *                                  belongs to, or {@code null} if the event is not
         *                                  associated with a specific media period.
         * @param eventPlaybackPositionMs   Position in the window or ad this event belongs to at
         *                                  the time of the event, in milliseconds.
         * @param currentPlaybackPositionMs Position in the current timeline window ({@link
         *                                  Player#getCurrentWindowIndex()}) or the currently
         *                                  playing ad at the time of the event, in milliseconds.
         * @param totalBufferedDurationMs   Total buffered duration from {@link #currentPlaybackPositionMs}
         *                                  at the time of the event, in milliseconds. This includes
     ...
     *
     * @param eventTime        The event time.
     * @param totalLoadTimeMs  The total time spend loading this update is based on, in
     *                         milliseconds.
     ...
     *
     * @param eventTime The event time.
     * @param width     The surface width in pixels. May be {@link C#LENGTH_UNSET} if unknown, or 0
     *                  if the video is not rendered onto a surface.
     * @param height    The surface height in pixels. May be {@link C#LENGTH_UNSET} if unknown, or 0
     *                  if the video is not rendered onto a surface.
     */
     ...
     *
     * @param eventTime       The event time.
     * @param trackType       The track type of the enabled decoder. Either {@link
     ...
     *
     * @param eventTime                The event time.
     * @param trackType                The track type of the initialized decoder. Either {@link
     *                                 C#TRACK_TYPE_AUDIO} or {@link C#TRACK_TYPE_VIDEO}.
     ...
     *
     * @param eventTime       The event time.
     * @param trackType       The track type of the disabled decoder. Either {@link
     *                        C#TRACK_TYPE_AUDIO} or {@link C#TRACK_TYPE_VIDEO}.
     * @param decoderCounters The accumulated event counters associated with this decoder.
     */
     ...
     * @param bufferSizeMs           The size of the {@link AudioSink}'s buffer, in milliseconds, if
     *                               it is configured for PCM output. {@link C#TIME_UNSET} if it is
     *                               configured for passthrough output, as the buffered media can
     *                               have a variable bitrate so the duration may be unknown.
     ...
     *                      duration is timed from when the renderer was started or from when
     *                      dropped frames were last reported (whichever was more recent), and not
     *                      from when the first of the reported drops occurred.
     */
     ...
     *                                 rotation in degrees that the application should apply for the
     *                                 video for it to be rendered in the correct orientation. This
     *                                 value will always be zero on API levels 21 and above, since
     *                                 the renderer will apply all necessary rotations internally.
     ...
     *
     * @param eventTime The event time.
     * @param surface   The {@link Surface} to which a first frame has been rendered, or {@code
     *                  null} if the renderer renders to something that isn't a {@link Surface}.
     */
    ...
    /**
     * Called when a drm error occurs. These errors are just for informational purposes and the
     * player may recover.
     *
     * @param eventTime The event time.
     * @param error     The error.
