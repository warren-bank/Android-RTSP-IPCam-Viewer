...
public final class PlaybackStats {
    ...
    /**
     * The total time spent joining the playback, in milliseconds, or {@link C#TIME_UNSET} if no
     * valid join time could be determined.
     *
     * <p>Note that this does not include background joining time. A join time may be invalid if
     * the
    ...
    /**
     * The number of playbacks with a valid join time as documented in {@link
     * #totalValidJoinTimeMs}.
     */
    ...
    /**
     * The maximum time spent during a single rebuffer, in milliseconds, or {@link C#TIME_UNSET} if
     * no rebuffer occurred.
     */
     ...
     * being used and the {@link Format}. The {@link Format} may be null if no video format was
     * used.
     */
     ...
     * being used and the {@link Format}. The {@link Format} may be null if no audio format was
     * used.
     */
    ...
    /**
     * The accumulated sum of all video format heights, in pixels, times the time the format was
     * used for playback, in milliseconds.
     */
    ...
    /**
     * The accumulated sum of all video format bitrates, in bits per second, times the time the
     * format was used for playback, in milliseconds.
     */
    ...
    /**
     * The accumulated sum of all audio format bitrates, in bits per second, times the time the
     * format was used for playback, in milliseconds.
     */
     ...
     * no valid join time is available. Only includes playbacks with valid join times as documented
     * in {@link #totalValidJoinTimeMs}.
     */
     ...
     * include invalid join times where the playback never reached {@link #PLAYBACK_STATE_PLAYING}
     * or {@link #PLAYBACK_STATE_PAUSED}, or joining was interrupted by a seek, stop, or error
     * state.
     */
    ...
    /**
     * Returns the mean time spent during a single rebuffer, in milliseconds, or {@link
     * C#TIME_UNSET} if no rebuffer was recorded. This excludes initial join times and buffer times
     * after a seek.
     */
    ...
    /**
     * Returns the mean time spent per foreground playback from the start of a seek until playback
     * is ready again, in milliseconds, or {@link C#TIME_UNSET} if no playback has been in
     * foreground.
     */
    ...
    /**
     * Returns the total time spent actively waiting for playback, in milliseconds. This includes
     * all join times, rebuffer times and seek times, but excludes times without user intention to
     * play, e.g. all paused states.
     */
     ...
     * milliseconds, or {@link C#TIME_UNSET} if no playback has been in foreground. This includes
     * all join times, rebuffer times and seek times, but excludes times without user intention to
     * play, e.g. all paused states.
     */
    ...
    /**
     * Returns the ratio of foreground playbacks which were abandoned before they were ready to
     * play, or {@code 0.0} if no playback has been in foreground.
     */
    ...
    /**
     * Returns the mean number of times a seek occurred per foreground playback, or {@code 0.0} if
     * no playback has been in foreground. This includes seeks happening before playback resumed
     * after another seek.
     */
    ...
    /**
     * Returns the mean number of times a rebuffer occurred per foreground playback, or {@code 0.0}
     * if no playback has been in foreground. This excludes initial joining and buffering after
     * seek.
     */
    ...
    /**
     * Returns the ratio of wait times to the total time spent playing and waiting, or {@code 0.0}
     * if no time was spend playing or waiting. This is equivalent to {@link #getTotalWaitTimeMs()}
    ...
    /**
     * Returns the ratio of rebuffer time to the total time spent playing and waiting, or {@code
     * 0.0} if no time was spend playing or waiting. This is equivalent to {@link
    ...
    /**
     * Returns the mean initial video format height, in pixels, or {@link C#LENGTH_UNSET} if no
    ...
    /**
     * Returns the mean initial video format bitrate, in bits per second, or {@link C#LENGTH_UNSET}
     * if no video format data is available.
     */
    ...
    /**
     * Returns the mean initial audio format bitrate, in bits per second, or {@link C#LENGTH_UNSET}
     * if no audio format data is available.
     */
     ...
     * data is available. This is a weighted average taking the time the format was used for
     * playback into account.
     */
     ...
     * video format data is available. This is a weighted average taking the time the format was
     * used for playback into account.
     */
     ...
     * audio format data is available. This is a weighted average taking the time the format was
     * used for playback into account.
     */
    ...
    /**
     * Returns the mean rate at which audio underruns occurred, in underruns per play time second,
     * or {@code 0.0} if no time was spent playing.
     */
    ...
    /**
     * Returns the ratio of foreground playbacks which experienced fatal errors, or {@code 0.0} if
    ...
    /**
     * Returns the rate of fatal errors, in errors per play time second, or {@code 0.0} if no time
     * was spend playing. This is equivalent to 1.0 / {@link #getMeanTimeBetweenFatalErrors()}.
     */
    ...
    /**
     * Returns the mean play time between fatal errors, in seconds. This is equivalent to 1.0 /
     * {@link #getFatalErrorRate()}. Note that this may return {@link Float#POSITIVE_INFINITY}.
     */
    ...
    /**
     * Returns the rate of non-fatal errors, in errors per play time second, or {@code 0.0} if no
     * time was spend playing. This is equivalent to 1.0 / {@link #getMeanTimeBetweenNonFatalErrors()}.
     */
