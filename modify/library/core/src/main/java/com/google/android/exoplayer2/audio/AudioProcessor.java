...
public interface AudioProcessor {
     ...
     * @throws UnhandledAudioFormatException Thrown if the specified format can't be handled as
     *                                       input.
     */
     ...
     * {@code buffer} must be a direct byte buffer with native byte order. Its contents are treated
     * as read-only. Its position will be advanced by the number of bytes consumed (which may be
     * zero). The caller retains ownership of the provided buffer. Calling this method invalidates
     * any previous buffer returned by {@link #getOutput()}.
     *
     ...
     * required to read all of the remaining output data. {@link #isEnded()} will return {@code
     ...
     * will always be a direct byte buffer with native byte order. Calling this method invalidates
     * any previously returned buffer. The buffer will be empty if no output is available.
     *
     ...
     * the audio processor to receive a new stream of input in the last configured (pending)
     * format.
     */
    void flush();
