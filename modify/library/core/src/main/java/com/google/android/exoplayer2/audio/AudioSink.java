...
public interface AudioSink {
    ...
    interface Listener {
        ...
        /**
         * Called when the audio sink handles a buffer whose timestamp is discontinuous with the
         * last buffer handled since it was reset.
         */
         ...
         * @param bufferSizeMs           The size of the sink's buffer, in milliseconds, if it is
         *                               configured for PCM output. {@link C#TIME_UNSET} if it is
         *                               configured for encoded audio output, as the buffered media
         *                               can have a variable bitrate so the duration may be
         *                               unknown.
         * @param elapsedSinceLastFeedMs The time since the sink was last fed data, in
         *                               milliseconds.
         */
     ...
     *                            sink's input as a preprocessing step, if handling PCM input.
     *                            Specify {@code null} to leave the input unchanged. Otherwise, the
     *                            element at index {@code i} specifies index of the input channel to
     *                            map to output channel {@code i} when preprocessing input buffers.
     *                            After the map is applied the audio data will have {@code
     *                            outputChannels.length} channels.
     * @param trimStartFrames     The number of audio frames to trim from the start of data written
     *                            to the sink after this call.
     * @param trimEndFrames       The number of audio frames to trim from data written to the sink
     *                            immediately preceding the next call to {@link #flush()} or this
     *                            method.
     ...
     * number of bytes that were handled. {@link Listener#onPositionDiscontinuity()} will be called
     * if {@code presentationTimeUs} is discontinuous with the last buffer handled since the last
     * reset.
     *
     * <p>Returns whether the data was handled in full. If the data was not handled in full then
     * the
     * same {@link ByteBuffer} must be provided to subsequent calls until it has been fully
     * consumed, except in the case of an intervening call to {@link #flush()} (or to {@link
     * #configure(int, int, int, int, int[], int, int)} that causes the sink to be flushed).
     *
     ...
     * the audio session id has changed. Enabling tunneling is only possible if the sink is based on
     * a platform {@link AudioTrack}, and requires platform API version 21 onwards.
     *
