...
/* package */ final class AudioTimestampPoller {
    ...
    /**
     * Polls the timestamp if required and returns whether it was updated. If {@code true}, the
     * latest timestamp is available via {@link #getTimestampSystemTimeUs()} and {@link
    ...
    /**
     * Rejects the timestamp last polled in {@link #maybePollTimestamp(long)}. The instance will
     * enter the error state and poll timestamps infrequently until the next call to {@link
     * #acceptTimestamp()}.
     */
     ...
     * #getTimestampSystemTimeUs()} and {@link #getTimestampSystemTimeUs()} to access the timestamp.
     * A current position for the track can be extrapolated based on elapsed real time since the
     * system time at which the timestamp was sampled.
     */
    ...
    /**
     * If {@link #maybePollTimestamp(long)} or {@link #hasTimestamp()} returned {@code true},
     * returns the system time at which the latest timestamp was sampled, in microseconds.
     */
    ...
    /**
     * If {@link #maybePollTimestamp(long)} or {@link #hasTimestamp()} returned {@code true},
     * returns the latest timestamp's position in frames.
     */
    ...
    private static final class AudioTimestampV19 {
         ...
         * updated, in which case the updated timestamp system time and position can be accessed
         * with {@link #getTimestampSystemTimeUs()} and {@link #getTimestampPositionFrames()}.
         * Returns {@code false} if no timestamp is available, in which case those methods should
         * not be called.
         */
