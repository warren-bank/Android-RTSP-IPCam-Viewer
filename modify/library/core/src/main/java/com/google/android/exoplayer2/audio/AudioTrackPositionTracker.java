...
/* package */ final class AudioTrackPositionTracker {
    ...
    public interface Listener {
         ...
         *                                     timestamp.
         * @param audioTimestampSystemTimeUs   The system time associated with the last known audio
         *                                     track timestamp, in microseconds.
         ...
         * @param bufferSizeMs The size of the sink's buffer, in milliseconds, if it is configured
         *                     for PCM output. {@link C#TIME_UNSET} if it is configured for encoded
         *                     audio output, as the buffered media can have a variable bitrate so
    ...
    /**
     * Checks the state of the audio track and returns whether the caller can write data to the
     * track. Notifies {@link Listener#onUnderrun(int, long)} if the track has underrun.
     *
    ...
    /**
     * Returns an estimate of the number of additional bytes that can be written to the audio
     * track's buffer without running out of space.
     *
     ...
     * playback head position as a long that will only wrap around if the value exceeds {@link
     * Long#MAX_VALUE} (which in practice will never happen).
     *
