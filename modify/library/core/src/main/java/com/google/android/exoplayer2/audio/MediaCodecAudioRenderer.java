 ...
 * message payload should be an {@link com.google.android.exoplayer2.audio.AudioAttributes} instance
 * that will configure the underlying audio track.
 * <li>Message with type {@link C#MSG_SET_AUX_EFFECT_INFO} to set the auxiliary effect. The
 * message payload should be an {@link AuxEffectInfo} instance that will configure the underlying
 * audio track.
 * </ul>
 */
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
     ...
     *                                    region so as to allow playback to begin in parallel with
     *                                    key acquisition. This parameter specifies whether the
     *                                    renderer is permitted to play clear regions of encrypted
     *                                    media files before {@code drmSessionManager} has obtained
     *                                    the keys necessary to decrypt encrypted regions of the
     *                                    media.
     * @deprecated Use {@link #MediaCodecAudioRenderer(Context, MediaCodecSelector, boolean,
     * Handler, AudioRendererEventListener, AudioSink)} instead, and pass DRM-related parameters to
     * the {@link MediaSource} factories.
     */
    @Deprecated
    @SuppressWarnings("deprecation")
     ...
     *                                    null if the default capabilities (no encoded audio
     *                                    passthrough support) should be assumed.
     * @param audioProcessors             Optional {@link AudioProcessor}s that will process PCM
     *                                    audio before output.
     * @deprecated Use {@link #MediaCodecAudioRenderer(Context, MediaCodecSelector, boolean,
     * Handler, AudioRendererEventListener, AudioSink)} instead, and pass DRM-related parameters to
     * the {@link MediaSource} factories.
     ...
     *                              initialization fails. This may result in using a decoder that is
     *                              slower/less efficient than the primary decoder.
     * @param eventHandler          A handler to use when delivering events to {@code
     *                              eventListener}. May be null if delivery of events is not
     *                              required.
     * @param eventListener         A listener of events. May be null if delivery of events is not
    ...
    /**
     * Returns a maximum input size suitable for configuring a codec for {@code format} in a way
    ...
    /**
     * Returns whether the decoder is known to output six audio channels when provided with input
     * with fewer than six channels.
     * <p>
    ...
    /**
     * Returns whether the decoder may output a non-empty buffer with timestamp 0 as the end of
     * stream buffer.
     *
