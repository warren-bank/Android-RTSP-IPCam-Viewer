...
public interface ExoMediaDrm<T extends ExoMediaCrypto> {
    ...
    interface Provider<T extends ExoMediaCrypto> {
        ...
        /**
         * Returns an {@link ExoMediaDrm} instance with an incremented reference count. When the
         * caller no longer needs to use the instance, it must call {@link ExoMediaDrm#release()} to
     ...
     *
     * @param scope              If {@code keyType} is {@link #KEY_TYPE_STREAMING} or {@link
     *                           #KEY_TYPE_OFFLINE}, the session id that the keys will be provided
     *                           to. If {@code keyType} is {@link #KEY_TYPE_RELEASE}, the keySetId
     *                           of the keys to release.
     * @param schemeDatas        If key type is {@link #KEY_TYPE_STREAMING} or {@link
     *                           #KEY_TYPE_OFFLINE}, a list of {@link SchemeData} instances
     *                           extracted from the media. Null otherwise.
     * @param keyType            The type of the request. Either {@link #KEY_TYPE_STREAMING} to
     *                           acquire keys for streaming, {@link #KEY_TYPE_OFFLINE} to acquire
     *                           keys for offline usage, or {@link #KEY_TYPE_RELEASE} to release
     *                           acquired keys. Releasing keys invalidates them for all sessions.
     ...
     *                           This may be {@code null} if no additional parameters are to be
     *                           sent.
     ...
     *
     * <p>A new instance will have an initial reference count of 1, and therefore it is not
     * normally
    ...
    /**
     * Returns the {@link ExoMediaCrypto} type created by {@link #createMediaCrypto(byte[])}, or
     * null if this instance cannot create any {@link ExoMediaCrypto} instances.
     */
    @Nullable
