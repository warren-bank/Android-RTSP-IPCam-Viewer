 ...
 *
 * <p>This interface provides two modes of accessing the underlying input. See the subheadings
 * below
...
public interface ExtractorInput {
     ...
     *                        allowed, and should result in {@code false} being returned. False if
     *                        it should be considered an error, causing an {@link EOFException} to
     *                        be thrown. See note in class Javadoc.
     ...
     * @throws EOFException         If the end of input was encountered having partially satisfied
     *                              the read (i.e. having read at least one byte, but fewer than
     *                              {@code length}), or if no bytes were read and {@code
     *                              allowEndOfInput} is false.
     * @throws IOException          If an error occurs reading from the input.
     * @throws InterruptedException If the thread has been interrupted.
     */
    ...
    /**
     * Like {@link #readFully(byte[], int, int, boolean)}, except the data is skipped instead of
     * read.
     *
     ...
     * @throws EOFException         If the end of input was encountered having partially satisfied
     *                              the skip (i.e. having skipped at least one byte, but fewer than
     *                              {@code length}), or if no bytes were skipped and {@code
     *                              allowEndOfInput} is false.
     * @throws IOException          If an error occurs reading from the input.
     * @throws InterruptedException If the thread has been interrupted.
     */
     ...
     * position, so the caller can peek the same data again. Reading or skipping also resets the
     * peek position.
     *
     * @param target          A target array into which data should be written.
     * @param offset          The offset into the target array at which to write.
     ...
     * @throws EOFException         If the end of input was encountered having partially satisfied
     *                              the peek (i.e. having peeked at least one byte, but fewer than
     *                              {@code length}), or if no bytes were peeked and {@code
     *                              allowEndOfInput} is false.
     ...
     * position, so the caller can peek the same data again. Reading and skipping also reset the
     * peek position.
     *
     * @param target A target array into which data should be written.
     * @param offset The offset into the target array at which to write.
     ...
     *                        and should result in {@code false} being returned. False if it should
     *                        be considered an error, causing an {@link EOFException} to be thrown.
     ...
     * data.
     * @throws EOFException         If the end of input was encountered having partially advanced
     *                              (i.e. having advanced by at least one byte, but fewer than
     *                              {@code length}), or if the end of input was encountered before
     *                              advancing and {@code allowEndOfInput} is false.
    ...
    /**
     * Called when reading fails and the required retry position is different from the last
     * position. After setting the retry position it throws the given {@link Throwable}.
     *
