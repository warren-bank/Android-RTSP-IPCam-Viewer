...
public class FragmentedMp4Extractor implements Extractor {
    ...
    /**
     * Flag to work around an issue in some video streams where every frame is marked as a sync
     * frame. The workaround overrides the sync frame flags in the stream, forcing them to false
     * except for the first sample in each segment.
     * <p>
    ...
    /**
     * @param flags                 Flags that control the extractor's behavior.
     * @param timestampAdjuster     Adjusts sample timestamps. May be null if no adjustment is
     *                              needed.
     * @param sideloadedTrack       Sideloaded track information, in the case that the extractor
     *                              will not receive a moov box in the input data. Null if a moov
     *                              box is expected.
     * @param sideloadedDrmInitData The {@link DrmInitData} to use for encrypted tracks. If null,
     *                              the pssh boxes (if present) will be used.
     */
    public FragmentedMp4Extractor(
            @Flags int flags,
     ...
     *                                  needed.
     * @param sideloadedTrack           Sideloaded track information, in the case that the extractor
     *                                  will not receive a moov box in the input data. Null if a
     *                                  moov box is expected.
     * @param sideloadedDrmInitData     The {@link DrmInitData} to use for encrypted tracks. If
     *                                  null, the pssh boxes (if present) will be used.
     * @param closedCaptionFormats      For tracks that contain SEI messages, the formats of the
     *                                  closed caption channels to expose.
     * @param additionalEmsgTrackOutput An extra track output that will receive all emsg messages
     *                                  targeting the player, even if {@link #FLAG_ENABLE_EMSG_TRACK}
     *                                  is not set. Null if special handling of emsg messages for
     ...
     * returns the {@link TrackBundle} of the corresponding {@link Track}. If the tfhd does not
     * refer to any {@link TrackBundle}, {@code null} is returned and no changes are made.
     *
     * @param tfhd         The tfhd atom to decode.
     * @param trackBundles The track bundles, one of which corresponds to the tfhd atom being
     *                     parsed.
     * @return The {@link TrackBundle} to which the {@link TrackFragment} belongs, or null if the
     * tfhd does not refer to any {@link TrackBundle}.
     */
     ...
     * skipped.
     *
     * <p>If there are no more samples in the current mdat atom then the parser state is
     * transitioned
     * to {@link #STATE_READING_ATOM_HEADER} and {@code false} is returned.
     *
     * <p>It is possible for a sample to be partially read in the case that an exception is thrown.
     * In
    ...
    /**
     * Returns the {@link TrackBundle} whose fragment run has the earliest file position out of
     * those yet to be consumed, or null if all have been consumed.
     */
    ...
    private static final class TrackBundle {
        ...
        /**
         * Advances {@link #firstSampleToOutputIndex} to point to the sync sample before the
         ...
         * If the current sample is the last one in the current fragment, then the advanced state
         * will be {@code currentSampleIndex == fragment.sampleCount}, {@code currentTrackRunIndex
