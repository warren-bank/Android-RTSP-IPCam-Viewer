...
public final class Mp4Extractor implements Extractor, SeekMap {
    ...
    /**
     * Creates a new extractor for unfragmented MP4 streams, using the specified flags to control
     ...
     * threshold {@link #RELOAD_MINIMUM_SEEK_DISTANCE}, {@code true} is returned and the caller
     * should restart loading at the position in {@code positionHolder}. Otherwise, the atom is
     * read/skipped.
     */
     ...
     *
     * <p>The preferred choice is the sample with the smallest offset not requiring a source
     * reload,
     ...
     * QuickTime do not have such a full box structure. Since some of these files are encoded
     * wrongly, we can't rely on the file type though. Instead we must check the 8 bytes after the
     * common header bytes ourselves.
     */
    ...
    /**
     * For each sample of each track, calculates accumulated size of all samples which need to be
     * read before this sample can be used.
     */
    ...
    /**
     * Adjusts a seek point offset to take into account the track with the given {@code
     * sampleTable}, for a given {@code seekTimeUs}.
     *
