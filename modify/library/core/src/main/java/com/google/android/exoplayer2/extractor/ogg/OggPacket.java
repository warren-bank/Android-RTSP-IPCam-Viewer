...
/* package */ final class OggPacket {
    ...
    /**
     * Reads the next packet of the ogg stream. In case of an {@code IOException} the caller must
     * make sure to pass the same instance of {@code ParsableByteArray} to this method again so this
     * reader can resume properly from an error while reading a continued packet spanned across
     * multiple pages.
     *
    ...
    /**
     * An OGG Packet may span multiple pages. Returns the {@link OggPageHeader} of the last page
     * read, or an empty header if the packet has yet to be populated.
     *
     * <p>Note that the returned {@link OggPageHeader} is mutable and may be updated during
     * subsequent
