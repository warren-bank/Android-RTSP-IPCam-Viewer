...
/* package */ final class PsBinarySearchSeeker extends BinarySearchSeeker {
     ...
     *
     * <p>Given a SCR timestamp, and a position within a PS stream, this seeker will peek up to
     * {@link
     * #TIMESTAMP_SEARCH_BYTES} bytes from that stream position, look for all packs in that range,
     * and then compare the SCR timestamps (if available) of these packets to the target timestamp.
     */
    private static final class PsScrSeeker implements TimestampSeeker {
        ...
        /**
         * Skips the buffer position to the position after the end of the current PS pack in the
         * buffer, given the byte position right after the {@link PsExtractor#PACK_START_CODE} of
         * the pack in the buffer. If the pack ends after the end of the buffer, skips to the end of
         * the buffer.
         */
