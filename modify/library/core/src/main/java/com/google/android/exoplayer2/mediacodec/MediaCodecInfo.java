...
public final class MediaCodecInfo {
    ...
    /**
     * The MIME type that the codec uses for media of type {@link #mimeType}, or {@code null} if
     * this is a passthrough codec. Equal to {@link #mimeType} unless the codec is known to use a
     ...
     *
     * <p>This could be an approximation as the exact information is only provided in API levels
     * 29+.
     *
     ...
     * @param codecMimeType        The MIME type that the codec uses for media of type {@code
     *                             #mimeType}. Equal to {@code mimeType} unless the codec is known
     *                             to use a non-standard MIME type alias.
     * @param capabilities         The capabilities of the {@link MediaCodec} for the specified mime
     *                             type, or {@code null} if not known.
     ...
     * @throws MediaCodecUtil.DecoderQueryException Thrown if an error occurs while querying
     *                                              decoders.
     */
     ...
     * configured to play media in the specified {@code format}. For adaptation to succeed, the
     * codec must also be configured with appropriate maximum values and {@link
    ...
    /**
     * Returns whether it is possible to adapt the decoder seamlessly from {@code oldFormat} to
     * {@code newFormat}. If {@code newFormat} may not be completely populated, pass {@code false}
     * for {@code isNewFormatComplete}.
     *
     ...
     * @param width  Width in pixels.
     * @param height Height in pixels.
     * @return The smallest video size greater than or equal to the specified size that also
     * satisfies the {@link MediaCodec}'s width and height alignment requirements, or null if not a
     * video codec.
     */
    @TargetApi(21)
