...
public abstract class MediaCodecRenderer extends BaseRenderer {
     ...
     * #codecAdaptationWorkaroundMode(String)}. Consists of three NAL units with start codes:
    ...
    /**
     * @param trackType                        The track type that the renderer handles. One of the
     *                                         {@code C.TRACK_TYPE_*} constants defined in {@link
     *                                         C}.
     * @param mediaCodecSelector               A decoder selector.
     * @param drmSessionManager                For use with encrypted media. May be null if support
     *                                         for encrypted media is not required.
     * @param playClearSamplesWithoutKeys      Encrypted media may contain clear (un-encrypted)
     *                                         regions. For example a media file may start with a
     *                                         short clear region so as to allow playback to begin
     *                                         in parallel with key acquisition. This parameter
     *                                         specifies whether the renderer is permitted to play
     *                                         clear regions of encrypted media files before {@code
     *                                         drmSessionManager} has obtained the keys necessary to
     *                                         decrypt encrypted regions of the media.
     * @param enableDecoderFallback            Whether to enable fallback to lower-priority decoders
     *                                         if decoder initialization fails. This may result in
     *                                         using a decoder that is less efficient or slower than
     *                                         the primary decoder.
     * @param assumedMinimumCodecOperatingRate A codec operating rate that all codecs instantiated
     *                                         by this renderer are assumed to meet implicitly (i.e.
     *                                         without the operating rate being set explicitly using
     *                                         {@link MediaFormat#KEY_OPERATING_RATE}).
     */
     ...
     * filling the decoder.
     *
     * <p>This method is experimental, and will be renamed or removed in a future release. It
     * should
    ...
    /**
     * Skip calling {@link MediaCodec#stop()} when the underlying MediaCodec is going to be
     * released.
     *
     * <p>By default, when the MediaCodecRenderer is releasing the underlying {@link MediaCodec},
     * it
     ...
     * @param crypto             For drm protected playbacks, a {@link MediaCrypto} to use for
     *                           decryption.
    ...
    /**
     * Determines whether the existing {@link MediaCodec} can be kept for a new {@link Format}, and
     * if it can whether it requires reconfiguration.
     *
     ...
     *
     * <p>When a new {@link ByteBuffer} is passed to this method its position and limit delineate
     * the
     ...
     *
     * @param positionUs               The current media time in microseconds, measured at the start
     *                                 of the current iteration of the rendering loop.
     * @param elapsedRealtimeUs        {@link SystemClock#elapsedRealtime()} in microseconds,
     *                                 measured at the start of the current iteration of the
     *                                 rendering loop.
     * @param codec                    The {@link MediaCodec} instance.
     ...
     * @param isDecodeOnlyBuffer       Whether the buffer was marked with {@link
     *                                 C#BUFFER_FLAG_DECODE_ONLY} by the source.
     ...
     *
     * <p>When enabled, the workaround queues and discards a blank frame with a resolution whose
     * width
     ...
     *
     * <p>If true is returned, the renderer will work around the issue by approximating end of
     * stream
     ...
     * channel.
     *
     * <p>If true is returned then we explicitly override the number of channels in the output
     * {@link
