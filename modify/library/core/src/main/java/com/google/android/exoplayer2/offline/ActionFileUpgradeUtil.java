...
public final class ActionFileUpgradeUtil {
    ...
    /**
     * Merges {@link DownloadRequest DownloadRequests} contained in a legacy action file into a
     * {@link DefaultDownloadIndex}, deleting the action file if the merge is successful or if
     * {@code deleteOnFailure} is {@code true}.
     *
     ...
     * @param downloadIdProvider         A download ID provider, or {@code null}. If {@code null}
     *                                   then ID of each download will be its custom cache key if
     *                                   one is specified, or else its URL.
