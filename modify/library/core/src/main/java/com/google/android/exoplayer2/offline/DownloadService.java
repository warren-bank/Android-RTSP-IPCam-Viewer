...
public abstract class DownloadService extends Service {
     ...
     *
     * <p>If {@code foregroundNotificationId} is not {@link #FOREGROUND_NOTIFICATION_ID_NONE} then
     * the
     ...
     *                                 {@link #FOREGROUND_NOTIFICATION_ID_NONE} if the service
     *                                 should only ever run in the background.
     */
     ...
     * Creates a DownloadService.
     *
     * @param foregroundNotificationId             The notification id for the foreground
     *                                             notification, or {@link #FOREGROUND_NOTIFICATION_ID_NONE}
     *                                             if the service should only ever run in the
     *                                             background.
     * @param foregroundNotificationUpdateInterval The maximum interval between updates to the
     *                                             foreground notification, in milliseconds. Ignored
     *                                             if {@code foregroundNotificationId} is {@link
     ...
     *                                             if {@code foregroundNotificationId} is {@link
     *                                             #FOREGROUND_NOTIFICATION_ID_NONE}.
     * @param channelId                            An id for a low priority notification channel to
     *                                             create, or {@code null} if the app will take care
     *                                             of creating a notification channel if needed. If
     *                                             specified, must be unique per package. The value
     *                                             may be truncated if it's too long. Ignored if
     *                                             {@code foregroundNotificationId} is {@link
     *                                             #FOREGROUND_NOTIFICATION_ID_NONE}.
     * @param channelNameResourceId                A string resource identifier for the user visible
     *                                             name of the notification channel. The recommended
     *                                             maximum length is 40 characters. The value may be
     *                                             truncated if it's too long. Ignored if {@code
     *                                             channelId} is null or if {@code foregroundNotificationId}
     ...
     *                                             description of the notification channel, or 0 if
     *                                             no description is provided. The recommended
     *                                             maximum length is 300 characters. The value may
     *                                             be truncated if it is too long. Ignored if {@code
     *                                             channelId} is null or if {@code foregroundNotificationId}
     *                                             is {@link #FOREGROUND_NOTIFICATION_ID_NONE}.
     */
    protected DownloadService(
     ...
     * @param foreground      Whether this intent will be used to start the service in the
     *                        foreground.
     ...
     * @param clazz           The concrete download service being targeted by the intent.
     * @param downloadRequest The request to be executed.
     * @param stopReason      An initial stop reason for the download, or {@link
     *                        Download#STOP_REASON_NONE} if the download should be started.
     * @param foreground      Whether this intent will be used to start the service in the
     *                        foreground.
     * @return The created intent.
     */
     ...
     *
     * <p>Download services that do not wish to run in the foreground should be created by setting
     * the
     * {@code foregroundNotificationId} constructor argument to {@link
     * #FOREGROUND_NOTIFICATION_ID_NONE}. This method will not be called in this case, meaning it
     * can be implemented to throw {@link UnsupportedOperationException}.
     *
