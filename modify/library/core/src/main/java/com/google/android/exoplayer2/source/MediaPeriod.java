...
public interface MediaPeriod extends SequenceableLoader {
    ...
    interface Callback extends SequenceableLoader.Callback<MediaPeriod> {
         ...
         *
         * <p>Called on the playback thread. After invoking this method, the {@link MediaPeriod}
         * can
     ...
     * becoming known), {@link MediaSourceCaller#onSourceInfoRefreshed(MediaSource, Timeline)} will
     * be called before {@code callback.onPrepared}.
     *
     ...
     *
     * <p>The call receives track {@code selections} for each renderer, {@code
     ...
     * provided selections, clearing, setting and replacing entries as required. If an existing
     * sample stream is retained but with the requirement that the consuming renderer be reset, then
     * the corresponding flag in {@code streamResetFlags} will be set to true. This flag will also
     * be set if a new sample stream is created.
     *
     * <p>Note that previously passed {@link TrackSelection TrackSelections} are no longer valid,
     * and
     ...
     *                             retained for each track selection. A {@code true} value indicates
     *                             that the selection is equivalent to the one that was previously
     *                             passed, and that the caller does not require that the sample
     *                             stream be recreated. If a retained sample stream holds any
     *                             references to the track selection then they must be updated to
     *                             point to the new selection.
     * @param streams              The existing sample streams, which will be updated to reflect the
     *                             provided selections.
     * @param streamResetFlags     Will be updated to indicate new sample streams, and sample
     *                             streams that have been retained but with the requirement that the
     *                             consuming renderer be reset.
     * @param positionUs           The current playback position in microseconds. If playback of
     *                             this period has not yet started, the value will be the starting
     *                             position.
     ...
     * @param toKeyframe If true then for each track discards samples up to the keyframe before or
     *                   at the specified position, rather than any sample before or at that
     *                   position.
     */
     ...
     * called when the period is permitted to continue loading data. A period may do this both
     * during and after preparation.
     *
     * @param positionUs The current playback position in microseconds. If playback of this period
     *                   has not yet started, the value will be the starting position in this period
     *                   minus the duration of any media in previous periods still to be played.
     ...
     * <p>This method is only called after the period has been prepared.
     *
     * <p>A period may choose to discard buffered media so that it can be re-buffered in a
     * different
     * quality.
     *
     * @param positionUs The current playback position in microseconds. If playback of this period
