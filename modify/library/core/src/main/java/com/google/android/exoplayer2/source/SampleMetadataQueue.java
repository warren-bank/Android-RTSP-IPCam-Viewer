...
/* package */ final class SampleMetadataQueue {
    ...
    /**
     * Returns whether a sample is available to be read. Internal methods.
     */
     ...
     *                        sample or the end of the stream. If a sample is read then the buffer
     *                        is populated with information about the sample, but not its data. The
     *                        size and absolute position of the data in the rolling buffer is stored
     *                        in {@code extrasHolder}, along with an encryption id if present and
     *                        the absolute position of the first byte that may still be required
     *                        after the current sample has been read. If a {@link
     *                        DecoderInputBuffer#isFlagsOnly() flags-only} buffer is passed, only
     *                        the buffer flags may be populated by this method and the read position
     *                        of the queue will not change. May be null if the caller requires that
     *                        the format of the stream be read even if it's not changing.
     * @param formatRequired  Whether the caller requires that the format of the stream be read even
     *                        if it's not changing. A sample will never be read if set to true,
     *                        however it is still possible for the end of stream or nothing to be
     *                        read.
     ...
     *                              sample (or keyframe) in the queue.
     ...
     * successful advance is one in which the read position was unchanged or advanced, and is now at
     * a sample meeting the specified criteria.
     */
     ...
     *                           specified time, rather than just any sample before or at that
     *                           time.
     ...
     * specified timestamp to be spliced in. Samples will not be discarded prior to the read
     * position.
     *
...
// Internal methods.
     ...
     * length, assuming that the timestamps prior to a keyframe are always less than the timestamp
     * of the keyframe itself, and of subsequent frames.
     *
     * @param length The length of the range being searched.
