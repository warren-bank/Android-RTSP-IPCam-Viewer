...
public class SampleQueue implements TrackOutput {
    private final static String TAG = "SampleQueue";
     ...
     *
     * @param allocator         An {@link Allocator} from which allocations for sample data can be
     *                          obtained.
     * @param drmSessionManager The {@link DrmSessionManager} to obtain {@link DrmSession
     *                          DrmSessions} from. The created instance does not take ownership of
     *                          this {@link DrmSessionManager}.
     */
     ...
     *                            samples queued after the reset (and before a subsequent call to
     *                            {@link #format(Format)}) are assumed to have the current upstream
     *                            format. If set to true, {@link #format(Format)} must be called
     *                            after the reset before any more samples can be queued.
     */
     ...
     *
     * @param discardFromIndex The absolute index of the first sample to be discarded. Must be in
     *                         the range [{@link #getReadIndex()}, {@link #getWriteIndex()}].
     */
     ...
     */
    //FIXME use isLastSampleQueued ?
    public boolean hasNextSample() {
        Log.e(TAG, "Show metadataQueue hasNextSample " + metadataQueue.hasNextSample());
        Log.e(TAG, "Show metadataQueue isLastSampleQueued " + metadataQueue.isLastSampleQueued());
        return metadataQueue.hasNextSample();
    }

    /**
     * Throws an error that's preventing data from being read. Does nothing if no such error
     * exists.
     *
     ...
     * @param toKeyframe         If true then discards samples up to the keyframe before or at the
     *                           specified time, rather than any sample before or at that time.
     * @param stopAtReadPosition If true then samples are only discarded if they're before the read
     *                           position. If false then samples at and beyond the read position may
     *                           be discarded, in which case the read position is advanced to the
     ...
     * @param toKeyframe            If true then attempts to advance to the keyframe before or at
     *                              the specified time, rather than to any sample before or at that
     *                              time.
     * @param allowTimeBeyondBuffer Whether the operation can succeed if {@code timeUs} is beyond
     *                              the end of the queue, by advancing the read position to the last
     *                              sample (or keyframe).
     ...
     * index is smaller than the index of the first sample in the queue, or larger than the index of
     * the next sample that will be written.
     */
     ...
     * See {@link DrmSessionManager#acquirePlaceholderSession(Looper, int)}.//FIXME
     * </ul>
     *
     * @param formatHolder      A {@link FormatHolder} to populate in the case of reading a format.
     * @param buffer            A {@link DecoderInputBuffer} to populate in the case of reading a
     *                          sample or the end of the stream. If the end of the stream has been
     *                          reached, the {@link C#BUFFER_FLAG_END_OF_STREAM} flag will be set on
     *                          the buffer. If a {@link DecoderInputBuffer#isFlagsOnly() flags-only}
     *                          buffer is passed, only the buffer flags may be populated by this
     *                          method and the read position of the queue will not change.
     * @param formatRequired    Whether the caller requires that the format of the stream be read
     *                          even if it's not changing. A sample will never be read if set to
     *                          true, however it is still possible for the end of stream or nothing
     *                          to be read.
     ...
     *
     * <p>Note: If the stream has ended then a buffer with the end of stream flag can always be
     * read
     * from {@link #read}. Hence an ended stream is always ready.
     *
     * @param loadingFinished Whether no more samples will be written to the sample queue. When
     *                        true, this method returns true if the sample queue is empty, because
     *                        an empty sample queue means the end of stream has been reached. When
     *                        false, this method returns false if the sample queue is empty.
     */
     ...
     *
     * @param absolutePosition The position to which {@link #readAllocationNode} should be
     *                         advanced.
     */
     ...
     *
     * @param absolutePosition The position to which {@link #firstAllocationNode} should be
     *                         advanced. May be {@link C#POSITION_UNSET}, in which case calling this
     *                         method is a no-op.
     */
    ...
    /**
     * Adjusts a {@link Format} to incorporate a sample offset into {@link
    ...
    private static final class AllocationNode {
        ...
        /**
         * Gets the offset into the {@link #allocation}'s {@link Allocation#data} that corresponds
