...
public class ChunkSampleStream<T extends ChunkSource> implements SampleStream, SequenceableLoader,
    ...
    /**
     * Selects the embedded track, returning a new {@link EmbeddedSampleStream} from which the
     * track's samples can be consumed. {@link EmbeddedSampleStream#release()} must be called on the
     * returned stream when the track is no longer required, and before calling this method again to
     * obtain another stream for the same track.
     *
     ...
     *                           index is required.
     * @param minChunkIndex      A minimum chunk index from which to start searching, or -1 if no
     *                           hint can be provided.
     * @return The index of the media chunk corresponding to the sample index, or -1 if the list of
     * media chunks is empty, or {@code minChunkIndex} if the sample precedes the first chunk in the
     * search (i.e. the chunk at {@code minChunkIndex}, or at index 0 if {@code minChunkIndex} is
     * -1.
     */
