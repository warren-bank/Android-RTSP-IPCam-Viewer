...
public interface ChunkSource {
     ...
     * <p>
     * Removing {@link MediaChunk}s from the back of the queue can be useful if they could be
     ...
     *
     * <p>If a chunk is available then {@link ChunkHolder#chunk} is set. If the end of the stream
     * has
     ...
     *                           period to which this chunk source belongs has not yet started, the
     *                           value will be the starting position in the period minus the
     *                           duration of any media in previous periods still to be played.
     * @param loadPositionUs     The current load position in microseconds. If {@code queue} is
     *                           empty, this is the starting position from which chunks should be
     *                           provided. Else it's equal to {@link MediaChunk#endTimeUs} of the
     *                           last chunk in the {@code queue}.
     * @param queue              The queue of buffered {@link MediaChunk}s.
     ...
     * @return Whether the load should be canceled so that a replacement chunk can be loaded
     * instead. Must be {@code false} if {@code cancelable} is {@code false}. If {@code true},
     * {@link #getNextChunk(long, long, List, ChunkHolder)} will be called to obtain the replacement
     * chunk.
     */
