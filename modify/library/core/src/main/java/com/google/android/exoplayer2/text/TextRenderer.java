...
public final class TextRenderer extends BaseRenderer implements Callback {
    ...
    /**
     * The decoder needs to be replaced, but we haven't yet signaled an end of stream to the
     * existing decoder. We need to do so in order to ensure that it outputs any remaining buffers
     * before we release it.
     */
    private static final int REPLACEMENT_STATE_SIGNAL_END_OF_STREAM = 1;
    /**
     * The decoder needs to be replaced, and we've signaled an end of stream to the existing
     * decoder. We're waiting for the decoder to output an end of stream signal to indicate that it
     * has output any remaining buffers before we release it.
     */
     ...
     *                     called. If the output makes use of standard Android UI components, then
     *                     this should normally be the looper associated with the application's main
     *                     thread, which can be obtained using {@link android.app.Activity#getMainLooper()}.
     *                     Null may be passed if the output should be called directly on the
     *                     player's internal rendering thread.
     */
     ...
     * @param outputLooper   The looper associated with the thread on which the output should be
     *                       called. If the output makes use of standard Android UI components, then
     *                       this should normally be the looper associated with the application's
     *                       main thread, which can be obtained using {@link android.app.Activity#getMainLooper()}.
     *                       Null may be passed if the output should be called directly on the
     *                       player's internal rendering thread.
