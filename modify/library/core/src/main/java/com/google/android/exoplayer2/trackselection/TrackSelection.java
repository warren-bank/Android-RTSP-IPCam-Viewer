...
public interface TrackSelection {
    ...
    final class Definition {
         ...
         * @param tracks The indices of the selected tracks within the {@link TrackGroup}. Must not
         *               be null or empty. May be in any order.
         */
         ...
         * @param tracks The indices of the selected tracks within the {@link TrackGroup}. Must not
         *               be
         * @param reason The track selection reason. One of the {@link C} SELECTION_REASON_
         *               constants.
    ...
    /**
     * Enables the track selection. Dynamic changes via {@link #updateSelectedTrack(long, long,
     * long, List, MediaChunkIterator[])} or {@link #evaluateQueueSize(long, List)} will only happen
     * after this call.
     *
    ...
    /**
     * Disables this track selection. No further dynamic changes via {@link
     * #updateSelectedTrack(long, long, long, List, MediaChunkIterator[])} or {@link
     * #evaluateQueueSize(long, List)} will happen after this call.
     *
     ...
     *
     * <p>This happens when the playback position jumps, e.g., as a result of a seek being
     * performed.
     */
     ...
     *                            period to which this track selection belongs has not yet started,
     *                            the value will be the starting position in the period minus the
     *                            duration of any media in previous periods still to be played.
     * @param bufferedDurationUs  The duration of media currently buffered from the current playback
     *                            position, in microseconds. Note that the next load position can be
     *                            calculated as {@code (playbackPositionUs + bufferedDurationUs)}.
     * @param availableDurationUs The duration of media available for buffering from the current
     *                            playback position, in microseconds, or {@link C#TIME_UNSET} if
     *                            media can be buffered to the end of the current period. Note that
     *                            if not set to {@link C#TIME_UNSET}, the position up to which media
     *                            is available for buffering can be calculated as {@code
     *                            (playbackPositionUs + availableDurationUs)}.
     * @param queue               The queue of already buffered {@link MediaChunk}s. Must not be
     *                            modified.
     * @param mediaChunkIterators An array of {@link MediaChunkIterator}s providing information
     *                            about the sequence of upcoming media chunks for each track in the
     *                            selection. All iterators start from the media chunk which will be
     *                            loaded next if the respective track is selected. Note that this
     *                            information may not be available for all tracks, and so some
     *                            iterators may be empty.
     */
     ...
     * queue. An example of a case where a smaller value may be returned is if network conditions
     * have improved dramatically, allowing chunks to be discarded and re-buffered in a track of
     * significantly higher quality. Discarding chunks may allow faster switching to a higher
     * quality track in this case. This method may only be called when the selection is enabled.
     *
     * @param playbackPositionUs The current playback position in microseconds. If playback of the
     *                           period to which this track selection belongs has not yet started,
     *                           the value will be the starting position in the period minus the
     ...
     * will remain selected until the next call to {@link #updateSelectedTrack(long, long, long,
