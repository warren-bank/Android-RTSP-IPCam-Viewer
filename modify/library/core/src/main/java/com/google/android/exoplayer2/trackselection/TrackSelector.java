 ...
 * #selectTracks(RendererCapabilities[], TrackGroupArray, MediaPeriodId, Timeline)}. This typically
 * occurs at the start of playback, when the player starts to buffer a new period of the media being
 * played, and when the track selector invalidates its previous selections.
 ...
 * corresponding to the selection that was made. For example when playing media containing multiple
 * periods, the track selection for a period is made when the player starts to buffer that period.
 * Hence if the player's buffering policy is to maintain a 30 second buffer, the selection will
 * occur approximately 30 seconds in advance of it becoming active. In fact the selection may never
 * become active, for example if the user seeks to some other period of the media during the 30
 * second gap. The player indicates to the track selector when a selection it has previously made
 * becomes active by calling {@link #onSelectionActivated(Object)}.
 ...
 * on the {@link InvalidationListener} that was passed to {@link #init(InvalidationListener,
 * BandwidthMeter)}. A track selector may wish to do this if its configuration has changed, for
 * example if it now wishes to prefer audio tracks in a particular language. This will trigger the
 * player to make new track selections. Note that the player will have to re-buffer in the case that
 * the new track selection for the currently playing period differs from the one that was
 * invalidated.
 * </ul>
 *
...
public abstract class TrackSelector {
    ...
    public interface InvalidationListener {
        ...
        /**
         * Called by a {@link TrackSelector} to indicate that selections it has previously made are
         * no longer valid. May be called from any thread.
         */
     ...
     *
     * @param listener       An invalidation listener that the selector can call to indicate that
     *                       selections it has previously made are no longer valid.
     * @param bandwidthMeter A bandwidth meter which can be used by track selections to select
     *                       tracks.
     */
     ...
     *                             tracks are to be selected.
     * @param trackGroups          The available track groups.
     * @param periodId             The {@link MediaPeriodId} of the period for which tracks are to
     *                             be selected.
     * @param timeline             The {@link Timeline} holding the period for which tracks are to
     *                             be selected.
     ...
     * #selectTracks(RendererCapabilities[], TrackGroupArray, MediaPeriodId, Timeline)} is
     * activated.
     *
    ...
    /**
     * Calls {@link InvalidationListener#onTrackSelectionsInvalidated()} to invalidate all
     * previously generated track selections.
     */
    ...
    /**
     * Returns a bandwidth meter which can be used by track selections to select tracks. Must only
     * be called after {@link #init(InvalidationListener, BandwidthMeter)} has been called.
     */
