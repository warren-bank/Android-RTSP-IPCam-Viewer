...
public interface DataSource {
     ...
     * (i.e. requests where {@link DataSpec#length} equals {@link C#LENGTH_UNSET}) this value is the
     * resolved length of the request, or {@link C#LENGTH_UNSET} if the length is still unresolved.
     * For all other requests, the value returned will be equal to the request's {@link
     * DataSpec#length}.
     * @throws IOException If an error occurs opening the source. {@link DataSourceException} can be
     *                     thrown or used as a cause of the thrown exception to specify the reason
     *                     of the error.
     */
     ...
     * index {@code offset}.
     *
     * <p>If {@code readLength} is zero then 0 is returned. Otherwise, if no data is available
     * because
     * the end of the opened range has been reached, then {@link C#RESULT_END_OF_INPUT} is returned.
     * Otherwise, the call will block until at least one byte of data has been read and the number
     ...
     * {@link Uri} will be identical to the one passed {@link #open(DataSpec)} in the {@link
     * DataSpec} unless redirection has occurred. If redirection has occurred, the {@link Uri} after
