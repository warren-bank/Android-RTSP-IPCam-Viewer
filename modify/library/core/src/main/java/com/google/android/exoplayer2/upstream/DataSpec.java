...
public final class DataSpec {
     ...
     * DataSource#open(DataSpec)} will typically be {@link C#LENGTH_UNSET}. The data read from
    ...
    /**
     * Allows fragmentation of this request into multiple cache files, meaning a cache eviction
     * policy will be able to evict individual fragments of the data. Depending on the cache
     * implementation, setting this flag may also enable more concurrent access to the data (e.g.
     * reading one fragment whilst writing another).
     */
    public static final int FLAG_ALLOW_CACHE_FRAGMENTATION = 1 << 2; // 4

    /**
     * The set of HTTP methods that are supported by ExoPlayer {@link HttpDataSource}s. One of
     * {@link #HTTP_METHOD_GET}, {@link #HTTP_METHOD_POST} or {@link #HTTP_METHOD_HEAD}.
     */
    @Documented
    @Retention(RetentionPolicy.SOURCE)
    ...
    /**
     * A key that uniquely identifies the original stream. Used for cache indexing. May be null if
     * the data spec is not intended to be used in conjunction with a cache.
     */
    @Nullable
    ...
    /**
     * Construct a data spec where {@link #position} may differ from {@link
     * #absoluteStreamPosition}.
     *
     * @param uri                    {@link #uri}.
     ...
     *
     * @param uri                    {@link #uri}.
     * @param postBody               {@link #httpBody} The body of the HTTP request, which is also
    ...
    /**
     * Construct a data spec with request parameters to be used as HTTP headers inside HTTP
     * requests.
     *
     * @param uri                    {@link #uri}.
     * @param httpMethod             {@link #httpMethod}.
     ...
     *
     * <p>Note: Values in {@code requestHeaders} will overwrite values with the same header key
     * that
