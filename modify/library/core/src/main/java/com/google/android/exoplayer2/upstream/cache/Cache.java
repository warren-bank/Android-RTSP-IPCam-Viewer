...
public interface Cache {
    ...
    interface Listener {
         ...
         * {@link CacheSpan} is guaranteed to represent the same data as the one it replaces,
         * however {@link CacheSpan#file} and {@link CacheSpan#lastTouchTimestamp} may have
         * changed.
         *
    ...
    /**
     * Returned by {@link #getUid()} if initialization failed before the unique identifier was read
     * or generated.
     */
    long UID_UNSET = -1;

    /**
     * Returns a non-negative unique identifier for the cache, or {@link #UID_UNSET} if
     * initialization failed before the unique identifier was determined.
     *
    ...
    /**
     * Releases the cache. This method must be called when the cache is no longer required. The
     * cache must not be used after calling this method.
     *
     ...
     *
     * <p>No guarantees are made about the thread or threads on which the listener is called, but
     * it
     * is guaranteed that listener methods will be called in a serial fashion (i.e. one at a time)
     ...
     *
     * <p>If there is no cache entry overlapping {@code offset}, then the returned {@link
     * CacheSpan}
     * defines a hole in the cache starting at {@code position} into which the caller may write as
     * it obtains the data from some other source. The returned {@link CacheSpan} serves as a lock.
     ...
     * by calling {@link #commitFile(File, long)}. When the caller has finished writing, it must
     ...
     *                 Used only to ensure that there is enough space in the cache.
    ...
    /**
     * Returns the length of the cached data block starting from the {@code position} to the block
     * end up to {@code length} bytes. If the {@code position} isn't cached then -(the length of the
     * gap to the next cached data up to {@code length} bytes) is returned.
     *
     * @param key      The cache key for the data.
     * @param position The starting position of the data.
