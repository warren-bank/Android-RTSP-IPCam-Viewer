...
/* package */ class CachedContentIndex {
    ...
    /**
     * Maps assigned ids to their corresponding keys. Also contains (id -> null) entries for ids
     * that have been removed from the index since it was last stored. This prevents reuse of these
     * ids, which is necessary to avoid clashes that could otherwise occur as a result of the
     * sequence:
     *
     ...
     * the index is stored to disk ... [4] The index is read from disk, causing the partially
     * written file to be incorrectly associated to key1
     *
     * <p>By avoiding id reuse in step [2], a new id2 will be used instead. Step [4] will then
     * delete
     * the partially written file because the index does not contain an entry for id2.
     *
     * <p>When the index is next stored (id -> null) entries are removed, making the ids eligible
     * for
     * reuse.
     */
     ...
     *
     * @param databaseProvider       Provides the database in which the index is stored, or {@code
     *                               null} to use only legacy storage.
     * @param legacyStorageDir       The directory in which any legacy storage is stored, or {@code
     *                               null} to use only database storage.
     * @param legacyStorageSecretKey A 16 byte AES key for reading, and optionally writing, legacy
     *                               storage.
     * @param legacyStorageEncrypt   Whether to encrypt when writing to legacy storage. Must be
     *                               false if {@code legacyStorageSecretKey} is null.
     * @param preferLegacyStorage    Whether to use prefer legacy storage if both storage types are
     *                               enabled. This option is only useful for downgrading from
     *                               database storage back to legacy storage.
     */
     ...
     * than {@link java.lang.Integer#MAX_VALUE} it just returns the next bigger integer. Otherwise
     * it returns the smallest unused non-negative integer.
     */
    @VisibleForTesting
    ...
    private interface Storage {
        ...
        /**
         * Loads the persisted index into {@code content} and {@code idToKey}, creating it if it
         * doesn't already exist.
         *
         * <p>If the persisted index is in a permanently bad state (i.e. all further attempts to
         * load it
         * are also expected to fail) then it will be deleted and the call will return successfully.
         * For transient failures, {@link IOException} will be thrown.
         *
        ...
        /**
         * Ensures incremental changes to the index since the initial {@link #initialize(long)} or
         * last {@link #storeFully(HashMap)} are persisted. The storage will have been notified of
         * all such changes via {@link #onUpdate(CachedContent)} and {@link #onRemove(CachedContent,
         * boolean)}.
         *
         * @param content The key to content map to persist.
         * @throws IOException If an error occurs persisting the index.
         ...
         * @param neverStored   True if the {@link CachedContent} was added more recently than when
         *                      the index was last stored.
         */
    ...
    private static class LegacyStorage implements Storage {
         ...
         * @param input   Input stream containing values needed to initialize CachedContent
         *                instance.
