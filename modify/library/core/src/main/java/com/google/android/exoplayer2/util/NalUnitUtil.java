...
public final class NalUnitUtil {
    ...
    /**
     * Temporary store for positions of escape codes in {@link #unescapeStream(byte[], int)}.
     * Guarded by {@link #scratchEscapePositionsLock}.
     */
    ...
    /**
     * Discards data from the buffer up to the first SPS, where {@code data.position()} is
     * interpreted as the length of the buffer.
     * <p>
     * When the method returns, {@code data.position()} will contain the new length of the buffer.
    ...
    /**
     * Parses an SPS NAL unit using the syntax defined in ITU-T Recommendation H.264 (2013)
    ...
    /**
     * Parses a PPS NAL unit using the syntax defined in ITU-T Recommendation H.264 (2013)
     * subsection 7.3.2.2.
     *
     ...
     * the detection of such NAL units. Note that when using this feature, the return value may be
     * 3, 2 or 1 less than {@code startOffset}, to indicate a NAL unit starting 3, 2 or 1 bytes
     ...
     *                    required to detect NAL units where the NAL unit prefix spans array
     *                    boundaries. The array must be at least 3 elements long.
