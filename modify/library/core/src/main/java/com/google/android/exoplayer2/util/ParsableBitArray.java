...
public final class ParsableBitArray {
    ...
    /**
     * Sets this instance's data, position and limit to match the provided {@code
     * parsableByteArray}. Any modifications to the underlying data array will be visible in both
     * instances
     *
     ...
     *
     * @param buffer  The array into which the read data should be written. The trailing {@code
     *                numBits % 8} bits are written into the most significant bits of the last
     *                modified {@code buffer} byte. The remaining ones are unmodified.
    ...
    /**
     * Aligns the position to the next byte boundary. Does nothing if the position is already
     * aligned.
     */
    ...
    /**
     * Reads the next {@code length} bytes into {@code buffer}. Must only be called when the
     ...
     *
     * @param value   The integer whose {@code numBits} least significant bits are written into
     *                {@link #data}.
