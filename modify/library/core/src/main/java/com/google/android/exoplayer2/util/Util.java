...
public final class Util {
     ...
     * @param uris     {@link Uri}s that may require {@link permission#READ_EXTERNAL_STORAGE} to
     *                 read.
     ...
     *                     index. If false then the returned index corresponds to the largest
     *                     element strictly less than the value.
     * @param stayInBounds If true, then 0 will be returned in the case that the value is smaller
     *                     than the smallest element in the array. If false then -1 will be
     *                     returned.
     ...
     * equal to) a specified {@code value}.
     *
     * <p>The search is performed using a binary search algorithm, so the list must be sorted. If
     * the
     ...
     *                     element strictly less than the value.
     * @param stayInBounds If true, then 0 will be returned in the case that the value is smaller
     *                     than the smallest element in the list. If false then -1 will be
     *                     returned.
     ...
     * @param value        The value being searched for.
     * @param inclusive    If the value is present in the array, whether to return the corresponding
     *                     index. If false then the returned index corresponds to the smallest
     *                     element strictly greater than the value.
     ...
     *                 output.data} isn't big enough to hold the uncompressed data, a new array is
     *                 created. If {@code true} is returned then the output's position will be set
     *                 to 0 and its limit will be set to the length of the uncompressed data.
    ...
    /**
     * Allows the CRC-32 calculation to be done byte by byte instead of bit per bit in the order
    ...
    /**
     * Allows the CRC-8 calculation to be done byte by byte instead of bit per bit in the order
     * "most significant bit first".
     */
