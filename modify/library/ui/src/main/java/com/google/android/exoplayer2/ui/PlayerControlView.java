 ...
 * being automatically hidden, in milliseconds. Use zero if the controls should not automatically
 * timeout.
 * <ul>
 ...
 * mode toggle options are enabled. Valid values are: {@code none}, {@code one}, {@code all}, or
 * {@code one|all}.
 * <ul>
...
public class PlayerControlView extends FrameLayout {
     ...
     *
     * @param player The {@link Player} to control, or {@code null} to detach the current player.
     *               Only players which are accessed on the main thread are supported ({@code
     ...
     *                            extraAdGroupTimesMs}, or {@code null} if {@code
     *                            extraAdGroupTimesMs} is {@code null}.
     */
     ...
     *                      controls to remain visible indefinitely.
     */
     ...
     *
     * <p>Note that smaller intervals, e.g. 33ms, will result in a smooth movement but will use
     * more
     * CPU resources while the time bar is visible, whereas larger intervals, e.g. 200ms, will
     * result in a step-wise update with less CPU usage.
     *
    ...
    /**
     * Shows the playback controls. If {@link #getShowTimeoutMs()} is positive then the controls
     * will be automatically hidden after this duration of time has elapsed without user input.
     */
    public void show() {
        if (!isVisible()) {
