...
public class PlayerNotificationManager {
    ...
    public interface MediaDescriptionAdapter {
         ...
         *
         * <p>When a bitmap initially needs to be asynchronously loaded, a placeholder (or null) can
         * be
         * returned and the bitmap asynchronously passed to the {@link BitmapCallback} once it is
         * loaded. Because the adapter may be called multiple times for the same media item, the
         * bitmap should be cached by the app and whenever possible be returned synchronously at
         * subsequent calls for the same media item.
         *
    ...
    public interface CustomActionReceiver {
         ...
         *
         * <p>If multiple {@link PlayerNotificationManager} instances are in use at the same time,
         * the
         ...
         * action receiver. It's also necessary to ensure something is different about the actions.
         * This may be any of the {@link Intent} attributes considered by {@link
         * Intent#filterEquals}, or different request code integers when creating the {@link
         * PendingIntent}s with {@link PendingIntent#getBroadcast}. The easiest approach is to use
         * the {@code instanceId} as the request code.
         *
     ...
     * @param channelName             A string resource identifier for the user visible name of the
     *                                notification channel. The recommended maximum length is 40
     *                                characters. The string may be truncated if it's too long.
     * @param channelDescription      A string resource identifier for the user visible description
     *                                of the notification channel, or 0 if no description is
     *                                provided. The recommended maximum length is 300 characters.
    ...
    /**
     * Creates a notification manager and a low-priority notification channel with the specified
     * {@code channelId} and {@code channelName}. The {@link NotificationListener} passed as the
     * last parameter will be notified when the notification is created and cancelled.
     *
     * @param context                 The {@link Context}.
     * @param channelId               The id of the notification channel.
     * @param channelName             A string resource identifier for the user visible name of the
     *                                channel. The recommended maximum length is 40 characters. The
     *                                string may be truncated if it's too long.
     * @param channelDescription      A string resource identifier for the user visible description
     *                                of the channel, or 0 if no description is provided.
     * @param notificationId          The id of the notification.
     * @param mediaDescriptionAdapter The {@link MediaDescriptionAdapter}.
     ...
     * NotificationListener} and {@link CustomActionReceiver}. The caller is responsible for
     ...
     * Player#STATE_IDLE}, in which case the notification is started as soon as the player
     * transitions away from being idle.
     *
     * <p>If the player is released it must be removed from the manager by calling {@code
     ...
     *
     * <p>Please note that you should call this method before you call {@link #setPlayer(Player)}
     * or
     ...
     *
     * @param fastForwardMs The fast forward increment in milliseconds. A value of zero will cause
     *                      the fast forward action to be disabled.
     */
     ...
     *
     * @param useNavigationActionsInCompactView Whether the navigation actions should be displayed
     *                                          in compact view.
     */
     ...
     *                 {@link NotificationCompat#PRIORITY_MAX}, {@link NotificationCompat#PRIORITY_HIGH},
     *                 {@link NotificationCompat#PRIORITY_LOW} or {@link NotificationCompat#PRIORITY_MIN}.
     *                 If not set {@link NotificationCompat#PRIORITY_LOW} is used by default.
     */
     ...
     *                   {@link NotificationCompat#VISIBILITY_PRIVATE} or {@link
     *                   NotificationCompat#VISIBILITY_SECRET}.
     */
     ...
     * @param builder   The builder used to build the last notification, or {@code null}. Re-using
     *                  the builder when possible can prevent notification flicker when {@code
     *                  Util#SDK_INT} &lt; 21.
     ...
     *
     * <p>This method can be overridden. The indices must refer to the list of actions passed as
     * the
     * first parameter.
     *
