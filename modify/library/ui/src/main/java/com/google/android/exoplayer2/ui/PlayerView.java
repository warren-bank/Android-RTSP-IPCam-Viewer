 ...
 * recommended for video applications. Note, TextureView can only be used in a hardware accelerated
 ...
 * also be set on a PlayerView, and will be propagated to the inflated {@link PlayerControlView}
 ...
 * this instance. If a view exists with this id, any {@code exo_controller_placeholder} view will be
 * ignored.
 * <ul>
...
public class PlayerView extends FrameLayout implements AdsLoader.AdViewProvider {
    ...
    /**
     * The buffering view is shown when the player is in the {@link Player#STATE_BUFFERING
     * buffering} state and {@link Player#getPlayWhenReady() playWhenReady} is {@code true}.
     */
     ...
     *
     * <p>To transition a {@link Player} from targeting one view to another, it's recommended to
     * use
     ...
     * <em>before</em> calling {@code setPlayer(null)} to detach it from the old one. This ordering
     * is significantly more efficient and may allow for more seamless transitions.
     *
     ...
     * controls are never visible and are disconnected from the player.
     *
     ...
     * different media, the player transitioning to unprepared media, {@link Player#stop(boolean)}
     * being called with {@code reset=true}, or the player being replaced or cleared by calling
     ...
     *
     * <p>If disabled, the currently displayed video frame or media artwork will be hidden as soon
     * as
     * the player is reset. Note that the video frame is hidden by making {@code exo_shutter}
     * visible. Hence the video frame will not be hidden if using a custom layout that omits this
     * view.
     *
     * @param keepContentOnPlayerReset Whether the currently displayed video frame or media artwork
     *                                 is kept visible when the player is reset.
     */
     ...
     * yet, is paused, has ended or failed.
     */
     ...
     *
     * @param controllerHideDuringAds Whether the playback controls are hidden when ads are
     *                                playing.
     */
     ...
     *
     * @param listener The listener to be notified about aspect ratios changes of the video content
    ...
    /**
     * Gets the overlay {@link FrameLayout}, which can be populated with UI elements to show on top
     * of the player.
     *
     * @return The overlay {@link FrameLayout}, or {@code null} if the layout has been customized
     * and the overlay is not present.
     */
    @Nullable
